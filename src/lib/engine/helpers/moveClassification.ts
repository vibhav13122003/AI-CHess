...
export const getMovesClassification = (
  ...
  let currentOpening: string | undefined = openingName;
  let currentEco: string | undefined;

  const positions = rawPositions.map((rawPosition, index) => {
    if (index === 0) return rawPosition;

    const playedMove = uciMoves[index - 1];
    const bookOpening = getEcoOpeningForFen(fens[index]);
    if (bookOpening) {
      currentOpening = bookOpening.name;
      currentEco = bookOpening.eco;
      return {
        ...rawPosition,
    ...
    // The recent-position path can start at either side to move. Derive the
    // mover from the previous FEN rather than the local slice index.
